...
def process_study(im0_file, label_files, reference_label):
    ct_data = None
    ...
    min_slice, max_slice = np.nonzero(reference_data)[2]
...
if __name__ == '__main__':
    ...
    studies = [each[:-4] for each in os.listdir(os.path.join(image_dir, 'SAT')) if each.endswith('BIM')]
    ...
    for study in studies:
        ...
        process_study(im0_file, label_files, labels[1])
        params.append((im0_file, label_files, labels[1]))
